...
# Create reversed video with different codec
reversed_video_path = "reversed_video.mp4"
ffmpeg_cmd = ['ffmpeg', '-i', video_path, '-c:v', 'libx265', '-vf', 'reverse', '-pix_fmt', 'yuv420p', reversed_video_path]
...
ffmpeg_cmd = ['ffmpeg', '-i', trimmed_video_path, '-i', audio_path, '-c:v', 'libx264', '-c:a', 'aac', '-map', '0:v:0', '-map', '1:a:0', output_path]
subprocess.run(ffmpeg_cmd)
