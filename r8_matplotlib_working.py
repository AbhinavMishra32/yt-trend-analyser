...
def plot_channel_histogram(channel_id, videos, title):
    ...
    plt.hist(view_counts, bins=20, color='blue', alpha=0.7)
    plt.title(title)
...
if __name__ == "__main__":
    ...
    if channel_ids:
        ...
        for channel_id, videos in videos_data.items():
            view_counts = [video['viewCount'] for video in videos]
            view_counts.sort(reverse=True)  # Sort view counts in descending order
            score = 0
            for i in range(1, len(view_counts)):
                if view_counts[i] > view_counts[i - 1]:
                    score += 1
            logger.info(f"Frequency distribution analysis for Channel {videos[0]['channel_name']} (ID: {channel_id}): Score = {score}")
    else:
        logger.warning("No channel IDs found. Exiting without plotting histograms.")
